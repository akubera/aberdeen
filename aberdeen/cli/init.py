...
from aberdeen.utils.error_messages import warning
...
def arg_parser():
    ...
    parser.add_argument("--defaults",
                        dest="interactive",
                        action="store_false",
                        help="Run with program without asking for user configuration.")
    parser.add_argument("-f", "--force",
                        dest="force",
                        action="store_true",
                        help="Will install regardless of files existing in directory.")
    parser.add_argument("dir_name",
                        help="The directory to install the files to")
    return parser
...
def main(argv=None):
    ...
    os.chdir(repo_name)
    repo_abs_path = Path(os.getcwd()).absolute().resolve()
    ...
    hook_path = repo_abs_path / 'hooks'

    hook_name = lambda fname: str(repo_abs_path / 'hooks' / fname)

    conf_path = hook_path / 'aberdeen.cfg'

    with conf_path.open('w') as cfg_file:
        cfg_file.write("# Automatically generated by the aberdeen-init script")
        cfg_file.write("\n\n")
        config.write(cfg_file)
    conf_path.chmod(0o640)
